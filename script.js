// const buttonAl = document.querySelector(".alert-btn"),
...
const range = document.querySelector(".custom-slider input"),
  ...
  setValue = () => {
    ...
    tooltip.innerHTML = range.value + " x " + range.value + " x ";
    tooltip.style.right = `calc(${newValue}% + (${newPosition}px))`;
  };
...
function fetchRequest(file, formData) {
  ...
  fetch("http://api.qrserver.com/v1/read-qr-code", {
    ...
    .catch(() => {
      reset();
      dropzoneText.innerText = "تعذر قرائة رمز QR";
    });
}
